...
export class HomePage implements OnInit {
  ...
  addMarker(location: any) {
    console.log('entrando a addmarker');
    if (this.markers.length < 2) {
      ...
      if (this.markers.length >= 1) {
        ...
        this.markerClickListener = googleMaps.event.addListener(marker, 'click', () => {
      ...
      return console.log('markers : ', this.markers);
    }
  ...
  this.mapClickListener = this.googleMaps.event.addListener(this.map, 'click', (mapsMouseEvent: any) => {
    ...
    // Llama al método para actualizar el segundo marcador
    this.updateSecondMarker();
  });
}
  ...
  async aplicar() {
    let navigationExtra: NavigationExtras={
      state:{precio: this.total,
          kmString2: this.kmString,
          km: this.km,
          capacidad: 4,
          lat1: this.app.lat,
          lat2: this.latt,
  ...
  updateSecondMarker() {
    if (this.markers.length === 2) {
      // Obtén el segundo marcador (el marcador en la posición 1 del array)
      const secondMarker = this.markers[1];
  
      // Actualiza la posición del segundo marcador con las nuevas coordenadas
      secondMarker.setPosition(new google.maps.LatLng(this.latt, this.lngg));
  
      // Vuelve a calcular la ruta
      this.calculateRoute();
  
      // Vuelve a pintar la ruta en el mapa
      this.directionsDisplay.setMap(this.map);
    }
  }

  calculateRoute() {
    console.log('entrando a calcular ruta')
    this.directionsService.route({
      ...
      if (status === google.maps.DirectionsStatus.OK) {
        ...
        this.km = Number(this.directionsDisplay.getDirections()?.routes[0].legs[0].distance?.value) / 1000;
